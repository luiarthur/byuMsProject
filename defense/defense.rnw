  ...
  \frame{
    ...
    \begin{itemize}
      ...
      \begin{itemize}
          ...
          \begin{itemize}
            \item $\bm{y = Zf + \epsilon}$, where $\bm Z$ is a binary matrix
...
\section{AIBP}  
  ...
  \frame{
    ...
    measures the proximity between customers $i$ and $j$,  $m_{-i}$ is the
    total number of dishes taken by all customers before customer $i$, and
    $p_{i,k} = \hik\frac{m_{-i}}{i}$.
  }
