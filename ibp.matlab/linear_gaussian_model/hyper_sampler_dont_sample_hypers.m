function [Z_sample, lP_sample, K_sample, alpha_sample, sigma_x_sample, sigma_A_sample] = hyper_sampler_dont_sample_hypers(X,num_samples,true_Z,true_alpha,true_sigma_x, true_sigma_A)
...
sigma_x_sample(1) = true_sigma_x;%true_sigma_x; %betarnd(1,1); % Beta(1,1) prior.  This could be rnd() as it Beta(1,1) is uniform over [0 1]
alpha_sample(1) = true_alpha;%gamrnd(1,1);   %<--- to ensure a Zsample in the beginning gamrnd(1,1); %Gamma(1,1) prior
sigma_A_sample(1) = true_sigma_A;%true_sigma_A; %rand(); % uniform over [0 1] prior
