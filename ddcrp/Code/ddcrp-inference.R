                        ...
                        clust.traj=FALSE, cust.traj=FALSE)
{
  ...
  ndata <- nrow(dat)
  msg.inc <- 10^(floor(log10(ndata))-1)
  if (clust.traj)
  ...
  st <- data.frame(idx=1:ndata, cluster=1:ndata, customer=1:ndata) # Every one in own cluster, connected to self
  lhood <- daply(st, .(cluster), function (df) lhood.fn(dat[df$idx,])) # splits data and applys fn by vars
                            ...
                            clust.traj=FALSE, cust.traj=FALSE)
{
  ...
  {
    msg(sprintf("document %d", i)) # hidden by arthur
  ...
  for (iter in 1:niter)
  {
    msg(sprintf("iter=%d", iter)) # blei
    #pb(iter,niter) #arthur

    iter.score <- 0
    for (i in seq(2,ndata)) # note: index i = 1 is correct at the outset
    {
      #if ((i %% msg.inc) == 0) msg(sprintf("%04d", i)) # arthur hid this
